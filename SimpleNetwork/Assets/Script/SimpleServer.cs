...
public class SimpleServer : MonoBehaviour {
    ...
    void Cleanup()
    {
        ...
        foreach (Socket con in m_Connections) con.Close();
        m_Connections.Clear();
    }
	...
	void Update () {
        ...
        if(m_Connections.Count != 0)
        {
            ...
            // Bring data from connected socket
            foreach(Socket socket in connections)
            {
                ...
                while(true && buffer.Count > 0)
                {
                    ...
                    if (length < buffer.Count)
                    {
                        ...
                        if (Singleton != this) Debug.LogError("Bug");
                    }
                    else break;
                }
            }
    ...
    {
        if (Singleton.m_Buffer.Count == 0) return null;
        else
        {
